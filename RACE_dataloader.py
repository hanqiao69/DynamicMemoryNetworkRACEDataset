...
class RACEDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            contexts, questions, answers = self.train_examples
        elif self.mode == 'valid':
            contexts, questions, answers = self.valid_examples
        elif self.mode == 'test':
            contexts, questions, answers = self.test_examples
        # print ('index debug: ', index, len(contexts), len(questions), len(answers))
        # return contexts[index], questions[index], answers[index]
        return contexts[index], torch.Tensor(questions[index]), torch.Tensor(answers[index])
...
if __name__ == '__main__':
    ...
    # train_examples = race_dataset.get_train_examples('.', 2)
